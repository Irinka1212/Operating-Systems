...
for file in "${@}"; do
	...
	if ! [[ "${file}" =~ ^([a-z0-9A-Z]+\.)+(\s+[0-9]+)?\s+IN\s+SOA\s+(([a-z0-9A-Z]+\.)+\s+){2}(\(|[0-9]{10}(\s+[0-9]+){4}) ]]; then
		continue
	fi

	line_SOA=$(grep "SOA" "${file}")
	if grep -q "(" "$line;do
		line_serial=$(echo "${line_SOA}" | tr -s ' ' | cut -d'(' -f2 | cut -d';' -f1 | tr -d' ')
	else
		second=$(echo "${line} | cut -d' ' -f2)
		if [[  "${second}" == "IN" ]];
			line_serial=$(echo "${line_SOA}" | tr -s ' ' | cut -d' ' -f4 | tr -d' ')
		else
			line_serial=$(echo "${line_SOA}" | tr -s ' ' | cut -d' ' -f5 | tr -d' ')
	fi

	if [[ ! "${line_serial}" =~ ^[[:digit:]]{10}$ ]]; then 
		continue
	fi

	line_serial_date=$(echo "${line_serial}" | cut -c1-8)
	today=$(date +'%Y%m%d')

	if [[ ${line_serial_date} -lt ${today} ]]; then
		new_line=$(${line_serial_date} +'00')
	elif [[ ${line_serial_date} -eq ${today} ]]; then
		new_line=$(( ${line_serial}+1 ))
		#expr ${line_serial}+1
		# echo ${line_serial}+1| bc 
	else
		continue
	fi

	sed -i "s/${line_serial}/${new_line}/" "${file}"
done