...
touch ${file_name}
echo "hostname,phy,vlans,hosts,failover,VPN-3DES-AES,peers,VLAN Trunk Ports,license,SN,key" >> ${file_name}
for file in "${dir}"/*.log; do
	file_info=$(cut -d':' -f2 "${file}" | cut -d' ' -f2 | tr '\n' ',')
	echo "$(basename "${file}")${file_info%,}" >> ${file_name}   		
done