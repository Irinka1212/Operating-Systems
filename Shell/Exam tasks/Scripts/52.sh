...
config_file=$1
while read line;do
	if ! [[ "${line}" =~ ^[A-Z0-9]{1,4}[\s\t]*(enabled|disabled) ]]; then
		continue
	fi
	...
	config_status=$(echo "${line}" | cut -d' ' -f2)

	name=$(egrep "^${config_name}" "${file}")
	if [[ -z "${name}" ]]; then
		continue
	fi
	
	status=$(echo "${name}" | cut -d' ' -f3)
	sed -i -E "s/^(${config_name}\s+[S][34])\s+\*(enabled|disabled)/\1 *${config_status}/" "${file}"
