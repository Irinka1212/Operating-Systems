// Зад. 79 2016-SE-02 Напишете програма на C, която реализира simple command prompt. Тя изпълнява в цикъл следната поредица действия:
...
int main(void) 
{
    while (1) 
    {
        ...
        num_bytes = read(0, command, sizeof(command));
        if (num_bytes <= 0) 
        {
            err(1, "error reading");
        }
